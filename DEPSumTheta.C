...
void DEPSumTheta()
{
  ...
  for (int i = 0; i < nrHistos; i++) {
     ...
     sprintf(SingleCMD,"fcsc*TMath::Sin(%.2f) + fesum*TMath::Cos(%.2f) >> %s",theta,theta,hSingleName);

     t->Draw(SingleCMD,"fcsc > 1000");
     ...
     double fitRange1 = (8400-cscOff)*TMath::Sin(theta) + 2280*TMath::Cos(theta);
     double fitRange2 = (10000-cscOff)*TMath::Sin(theta) + 2600*TMath::Cos(theta);
     double Peak = (9000-cscOff)*TMath::Sin(theta) + 2350*TMath::Cos(theta);
  ...
  t->Draw("fcsc:fesum>>h2DSingle","fcsc > 1000");
  ...
  TF1 *fitRes = new TF1("fitRes","pol2",0.1,0.3);
