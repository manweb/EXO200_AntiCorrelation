...
*/
  ...
  TFile *fOut = new TFile("../analysis/Co60_scint_PeakPosition.root","RECREATE");
  gr1->Write();
  gr2->Write();
  gr3->Write();
  gr4->Write();
...
*/
  ...
  TH1F *hMean = new TH1F("hMean","Combined energy spectrum (single site)",200,0,10000);
  TH1F *hMulti = new TH1F("hMulti","Combined energy spectrum (multi site)",200,0,10000);

  t->Draw("csc>>hMean","nsite == 1");
  //t->Draw("ecrec>>hMean","nsite == 1 && TMath::Abs(zc) < 150");
  t->Draw("csc>>hMulti","nsite > 1");

  TF1 *fit = new TF1("fit",fitFunction,3200,6000,7);
  fit->SetParNames("A1","E1","#sigma","R1","R2","E2","#sigma2");

  fit->SetParameters(500,4300,200,0.6,0.8,4700,200);

  fit->SetParLimits(1,4000,4500);
  //fit->SetParLimits(1,1000,1250);
  fit->SetParLimits(2,100,700);
  fit->SetParLimits(3,0.05,1.0);
  //fit->SetParLimits(3,0.2,5);
  fit->SetParLimits(4,0.75,1.2);
  fit->SetParLimits(5,4650,5000);
    fit->SetParLimits(6,100,700);
  //fit->SetParLimits(5,1200,1400);

  hMean->Fit("fit","r");

  TF1 *fitMulti = new TF1("fitMulti",fitFunction,3200,6000,7);
  ...
  fitMulti->SetParameters(500,4300,200,0.2,0.8,4700,200);
  ...
  //fitMulti->SetParLimits(1,1100,1250);
  fitMulti->SetParLimits(1,4000,4500);
  fitMulti->SetParLimits(2,100,800);
  fitMulti->SetParLimits(3,0.05,1.0);
  fitMulti->SetParLimits(4,0.5,1.2);
  fitMulti->SetParLimits(5,4650,5100);
  fitMulti->SetParLimits(6,100,800);
