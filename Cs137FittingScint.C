...
*/
  ...
  TFile *fOut = new TFile("../analysis/Cs137_scint_PeakPosition_all.root","RECREATE");
  gr1->Write();
  gr2->Write();
...
*/
  ...
  TH1F *hMean = new TH1F("hMean","Combined energy spectrum (single site)",200,0,8000);
  TH1F *hMulti = new TH1F("hMulti","Combined energy spectrum (multi site)",200,0,8000);
  ...
  t->Draw("csc*TMath::Sin(0.1944) + ecrec*TMath::Cos(0.1944)>>hMean","nsite == 1 && algsc == 0");
  t->Draw("csc*TMath::Sin(0.1944) + ecrec*TMath::Cos(0.1944)>>hMulti","nsite > 1 && algsc == 0");

  TF1 *fit = new TF1("fit",fitFunction,790,1370,4);
  fit->SetParNames("A1","E1","#sigma","A2");

  fit->SetParameters(800,1070,100,0.8);

  fit->SetParLimits(1,900,1200);
  fit->SetParLimits(2,50,200);
  fit->SetParLimits(3,0.05,2.0);

  hMean->Fit("fit","r");

  TF1 *fitMulti = new TF1("fitMulti",fitFunction,790,1370,4);
  //TF1 *fitMulti = new TF1("fitMulti","gaus",530,650);
  fitMulti->SetParNames("A","E","#sigma","A2");

  fitMulti->SetParameters(800,1070,100,0.2);

  fitMulti->SetParLimits(1,900,1200);
